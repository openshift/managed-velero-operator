...
if [[ "$REMOVE_UNDEPLOYED" == true ]]; then
    DEPLOYED_HASH=$(
        ...
        docker run --rm -i quay.io/app-sre/yq -r '.resourceTemplates[]|select(.name="managed-velero-operator").targets[]|select(.namespace["$ref"]=="/services/osd-operators/namespaces/hive-production-cluster-scope.yml")|.ref'
    )
