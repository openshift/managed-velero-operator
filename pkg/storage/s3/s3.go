...
func (d *driver) CreateStorage(reqLogger logr.Logger, instance *veleroInstallCR.VeleroInstall) error {
	...
	bucketLog := reqLogger.WithValues("StorageBucket.Name", instance.Status.AWS.StorageBucket.Name, "StorageBucket.Region", d.Config.Region)

	// This switch handles the provisioning steps/checks
	switch {
	// We don't yet have a bucket name selected
	case instance.Status.AWS.StorageBucket.Name == "":
	...
	case instance.Status.AWS.StorageBucket.Name != "" && !instance.Status.AWS.StorageBucket.Provisioned:
		bucketLog.Info("S3 bucket defined, but not provisioned")

		// Create S3 bucket
		bucketLog.Info("Creating S3 Bucket")
		err = CreateBucket(s3Client, instance.Status.AWS.StorageBucket.Name)
		if err != nil {
			if aerr, ok := err.(awserr.Error); ok {
				switch aerr.Code() {
				case s3.ErrCodeBucketAlreadyExists:
					bucketLog.Info("Bucket exists, but is not owned by current user; retrying")
					instance.Status.AWS.StorageBucket.Name = ""
				...
				default:
					return fmt.Errorf("error occurred when creating bucket %v: %v", instance.Status.AWS.StorageBucket.Name, aerr.Error())
				}
			} else {
				return fmt.Errorf("error occurred when creating bucket %v: %v", instance.Status.AWS.StorageBucket.Name, err.Error())
			}
		}
		err = TagBucket(s3Client, instance.Status.AWS.StorageBucket.Name, storageConstants.DefaultVeleroBackupStorageLocation, d.Config.InfraName)
		if err != nil {
			return fmt.Errorf("error occurred when tagging bucket %v: %v", instance.Status.AWS.StorageBucket.Name, err.Error())
		}
	}
	...
	if err != nil {
		...
		return fmt.Errorf("error occurred when verifying bucket %v: %v", instance.Status.AWS.StorageBucket.Name, err.Error())
	}
	if !exists {
		bucketLog.Error(nil, "S3 bucket doesn't appear to exist")
		instance.Status.AWS.StorageBucket.Provisioned = false
		return instance.StatusUpdate(reqLogger, d.KubeClient)
	}

	// Encrypt S3 bucket
	bucketLog.Info("Enforcing S3 Bucket encryption")
	err = EncryptBucket(s3Client, instance.Status.AWS.StorageBucket.Name)
	if err != nil {
		if aerr, ok := err.(awserr.Error); ok {
			return fmt.Errorf("error occurred when encrypting bucket %v: %v", instance.Status.AWS.StorageBucket.Name, aerr.Error())
		}
		return fmt.Errorf("error occurred when encrypting bucket %v: %v", instance.Status.AWS.StorageBucket.Name, err.Error())
	}

	// Block public access to S3 bucket
	bucketLog.Info("Enforcing S3 Bucket public access policy")
	err = BlockBucketPublicAccess(s3Client, instance.Status.AWS.StorageBucket.Name)
	if err != nil {
		if aerr, ok := err.(awserr.Error); ok {
			return fmt.Errorf("error occurred when blocking public access to bucket %v: %v", instance.Status.AWS.StorageBucket.Name, aerr.Error())
		}
		return fmt.Errorf("error occurred when blocking public access to bucket %v: %v", instance.Status.AWS.StorageBucket.Name, err.Error())
	}

	// Configure lifecycle rules on S3 bucket
	bucketLog.Info("Enforcing S3 Bucket lifecycle rules on S3 Bucket")
	err = SetBucketLifecycle(s3Client, instance.Status.AWS.StorageBucket.Name)
	if err != nil {
		if aerr, ok := err.(awserr.Error); ok {
			return fmt.Errorf("error occurred when configuring lifecycle rules on bucket %v: %v", instance.Status.AWS.StorageBucket.Name, aerr.Error())
		}
		return fmt.Errorf("error occurred when configuring lifecycle rules on bucket %v: %v", instance.Status.AWS.StorageBucket.Name, err.Error())
	}
	...
	instance.Status.AWS.StorageBucket.Provisioned = true
	instance.Status.AWS.StorageBucket.LastSyncTimestamp = &metav1.Time{
		Time: time.Now(),
	}
	return instance.StatusUpdate(reqLogger, d.KubeClient)
...
func setInstanceBucketName(d *driver, s3Client Client, reqLogger logr.Logger, instance *veleroInstallCR.VeleroInstall) error {
	...
	if existingBucket != "" {
		...
		instance.Status.AWS.StorageBucket.Name = existingBucket
		instance.Status.AWS.StorageBucket.Provisioned = true
		return instance.StatusUpdate(reqLogger, d.KubeClient)
	}
	...
	instance.Status.AWS.StorageBucket.Name = proposedName
	instance.Status.AWS.StorageBucket.Provisioned = false
	return instance.StatusUpdate(reqLogger, d.KubeClient)
}
